...
class UserKYCController extends getx.GetxController {
  ...
  void pickCitizenshipFront() async {
    citizenshipFront = await _picker.pickMultiImage();
    update();
  }

  void pickCitizenshipBack() async {
    citizenshipBack = await _picker.pickImage(source: ImageSource.gallery);
    update();
  }
  ...
  void submitKYC() async {
    ...
    if (citizenshipFront == null) {
      getx.Get.rawSnackbar(
          ...
          message: "Some of the document is missing");
      return;
    }
    isLoading = true;
    update();
